import {
  getIsKeyButtonActive,
  setIsKeyButtonActive,
  setSortDirection,
  setSortKey,
  useAppDispatch,
  useAppSelector,
...
export const PlatformsWorkHeaderPanel = () => {
  const {
    register,
    formState: { errors },
    getValues,
  ...
  const isKeyButtonActive = useAppSelector(getIsKeyButtonActive);
  ...
  const handleClickSortKey = () => {
    ...
    if (sortKeyQuery !== sortKey.trim()) {
      ...
      dispatch(setIsKeyButtonActive(false));
    }
  };
  ...
  useEffect(() => {
    dispatch(setSortKey(sortKeyQuery));
  }, [sortKeyQuery, dispatch]);

  // обновление sortDirection в store при его изменении в query
  useEffect(() => {
    dispatch(setSortDirection(sortDirectionQuery));
  }, [sortDirectionQuery, dispatch]);

  const handleClick = () => {
    const newPath = `${ROUTE.ADMIN}/${ROUTE.PLATFORMS}/${AdminPlatformRoutes.create}`;
    navigate(newPath);
  };
